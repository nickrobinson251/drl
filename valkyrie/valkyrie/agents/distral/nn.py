...
from valkyrie.utils import Serializable, Parameterised
...
def feedforward_net(inputs,
    ...
    def linear(x, n_units, name=None):
        input_size = x.shape[-1].value
        weight_name = 'weight' + '_' + str(name) if name else 'weight'
...
class MLPFunction(Parameterised, Serializable):
