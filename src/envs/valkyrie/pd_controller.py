...
class PDController:
    ...
    def updateMeasurements(self, measured_position, measured_velocity):
        if self.is_filter[0]:
            ...
            self.filtered_position = self.position_filter.applyFilter(measured_position)
            self.adjusted_position = self.filtered_position + (self.filtered_position-past_position)
            self.measured_position = measured_position
            self.kalman_filtered_position = self.kalman_position_filter.input_latest_noisy_measurement(measured_position)
            self.kalman_filtered_velocity = self.kalman_velocity_filter.input_latest_noisy_measurement(measured_velocity)
        else:
            past_position = self.filtered_position
        ...
        if self.is_filter[1]:
            ...
            self.filtered_velocity = self.velocity_filter.applyFilter(measured_velocity)
            self.adjusted_velocity = self.filtered_velocity + (self.filtered_velocity-past_velocity)
