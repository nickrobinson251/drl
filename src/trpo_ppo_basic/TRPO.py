...
class TRPO:
    def __init__(self, config, state_dim, action_dim):
        ...
        self.actor_init_weight=[0.1,0.1,0.1,0.01]#np.sqrt(1.0/np.concatenate([[self.actor_input_dim], self.actor_layer_dim],axis=0))#[0.1,0.1,0.1,0.01]
        ...
        self.critic_init_weight=np.sqrt(2.0/np.concatenate([[self.critic_input_dim], self.actor_layer_dim],axis=0))#[0.1,0.1,0.1,0.1]
        self.critic_init_bias=[None,None,None,None]
        self.critic_activation=self.config.conf['critic-activation-fn']
        with tf.variable_scope("agent"):

            with tf.variable_scope('actor_network'):
                self.actor_network = Fcnn(sess=self.sess, input_dim=self.state_dim, output_dim=self.action_dim,
            ...
            with tf.variable_scope('old_actor_network'):
                self.old_actor_network = Fcnn(sess=self.sess, input_dim=self.state_dim, output_dim=self.action_dim,
                                            layer_dim=self.actor_layer_dim, config=self.config, if_bias=self.actor_bias,
                                            activation=self.actor_activation, init_weight=self.actor_init_weight,
                                            init_bias=self.actor_init_bias, input_tf=self.state_input['actor'],
            ...
            with tf.variable_scope('critic_network'):
                self.critic_network = Fcnn(sess=self.sess, input_dim=self.state_dim, output_dim=1,
    ...
    def setup_critic_training_method(self):
        ...
        #L2 regularization
        weight_decay = tf.add_n([self.critic_l2 * tf.nn.l2_loss(var) for var in self.critic_trainable_vars])
        ...
        if self.opt_method['critic']['critic-l2']:
            ...
            self.V_optimizer = tf.train.AdamOptimizer(self.opt_method['critic']['critic-lr']).minimize(self.vloss)
    ...
    def setup_actor_training_method(self):
        ...
        self.surr_loss = tf.reduce_mean(tf.maximum(self.surr_loss1, self.surr_loss2))# + self.config.conf['loss-entropy-coeff']*self.entropy_loss
        ...
        grads = tf.gradients(kl_sym(self.old_dist_mean, self.old_dist_logstd, self.new_dist_mean, self.new_dist_logstd), self.actor_trainable_vars)

        flat_grads = flatten_var(grads) / batchsize
        self.fvp = tf.gradients(tf.reduce_sum(flat_grads * self.flat_tangent), self.actor_trainable_vars)
        self.flat_fvp = flatten_var(self.fvp)

        self.weights_to_set = tf.placeholder(tf.float32, [None], name='weights_to_set')
        self.set_var_from_flat = set_from_flat(self.actor_trainable_vars, self.weights_to_set)
        self.flatten_var = flatten_var(self.actor_trainable_vars)

        #PPO loss
        self.PPO_loss = self.surr_loss + self.config.conf['loss-entropy-coeff']*self.entropy_loss
    ...
    def train_actor_TRPO(self, on_policy_state, on_policy_action, on_policy_advantage, old_dist_mean, old_dist_logstd):
        ...
        def fisher_vector_product(p):
            ...
            return self.sess.run(self.flat_fvp, feed_dict=feed_dict) + self.config.conf['actor-opt-method']['cg-damping'] * p

        g = self.sess.run(self.flat_surr_grad, feed_dict=feed_dict)
        step_gradient = cg(fisher_vector_product, -g, cg_iters=self.config.conf['actor-opt-method']['conjugate-gradient-iteration'])
        sAs = step_gradient.dot(fisher_vector_product(step_gradient))
        inv_stepsize = np.sqrt(sAs / (2. * self.config.conf['actor-opt-method']['epsilon']))
        stepsize = np.sqrt((2.*self.config.conf['actor-opt-method']['epsilon']) / (sAs+1e-8))
        print('g', sum(g))
        ...
        if self.config.conf['actor-opt-method']['line-search']:
            flat_theta_new = linesearch(loss_function, flat_theta_prev, fullstep_gradient, self.config.conf['actor-opt-method']['line-search-backtrack'],
