...
class Net(object):
    def __init__(self,
        ...
        self.all_layer_dim = np.concatenate([
            [self.input_dim], layer_dim, [self.output_dim]], axis=0).astype(int)
        self.if_bias = kwargs.get(
            'if_bias', ([True] * len(layer_dim)) + [False])
        self.activation_fns = kwargs.get(
            'activation', (['tanh'] * len(layer_dim)) + ['None'])
        ...
        act_funcs_dict = {
            ...
            'relu': tf.nn.relu,
            'None': lambda x: x}
        self.activation_fns_call = [act_funcs_dict[_]
                                    for _ in self.activation_fns]
    ...
    def build(self, input_tf, name):
        ...
        for i, (dim_1, dim_2) in enumerate(
            ...
            if self.if_bias[i]:
                ...
                weight = tf.get_variable(initializer=tf.truncated_normal(
                                             [dim_1, dim_2],
                                             stddev=init_w),
                                         trainable=self.trainable,
                                         dtype=tf.float32, name='theta_%i' % i)
                weights.append(weight)
                # zero initialization for bias
                init_b = self.initialize_bias[i]
                if init_b is None:
                    bias = tf.get_variable(
                        initializer=np.zeros([1, dim_2]).astype(np.float32),
