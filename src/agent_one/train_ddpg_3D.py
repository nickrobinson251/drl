    ...
    for episode in range(EPISODES):
        ...
        while 1 > 0:  # infinite loop
            ...
            for rollout in range(config.conf['rollout-step-num']):
                ...
                if external_force_disturbance:
                    if step == network_frequency or step == 6 * network_frequency or step == 11 * \
                            network_frequency:  # apply force for every 5 second
                ...
                # abs(prev_action[1] - action[1]) + abs(prev_action[2] -
                # action[2]) + abs(prev_action[3] - action[3]))
