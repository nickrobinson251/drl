...
class DDPG:
    ...
    def __init__(self, env, config):
        ...
        self.actor_output_bounds = config.conf['actor-output-bound']#config.conf['normalized-action-bounds']
...
#        self.sess = tf.InteractiveSession(config=config)
        ...
        if self.config.conf['noise-normalization'] == True:
            self.exploration_noise = OUNoise(self.action_dim, self.config.conf['OU-noise-settings'][1]*self.config.conf['action-scale'], self.config.conf['OU-noise-settings'][2]*self.config.conf['action-scale'])
        else:
    ...
    def train(self):
        ...
        for i in range(0, train_num):
            ...
            if self.config.conf['prioritized-exp-replay'] ==True:
                ...
                loss, abs_errors = self.critic_network.train(y_batch, state_batch, action_batch, ISWeights)
                #print(abs_errors)
                self.memory.batch_update(tree_idx, abs_errors)
            else:
                loss = self.critic_network.train(y_batch, state_batch, action_batch, [])
    ...
    def action_noise(self, state, epsilon=1, lamda=1):
        ...
        if self.config.conf['OU-noise'] == True:
            ...
            #         ou_noise[i] *= abs(self.config.conf['action-bounds'][1][i]-self.config.conf['action-bounds'][0][i])/2.0

            action = action + ou_noise

        #translate action from [-1.0,1.0] to joint range
        for i in range(self.action_dim):
            action[i] = translate(action[i], [self.actor_output_bounds[0][i], self.actor_output_bounds[1][i]], [self.action_bounds[0][i], self.action_bounds[1][i]])
    ...
    def actions_target(self, state):
        ...
        for i in range(self.action_dim):
            # translate action from [-1.0,1.0] to joint range
            action_temp = translate(actions[:,i], [self.actor_output_bounds[0][i], self.actor_output_bounds[1][i]], [self.action_bounds[0][i], self.action_bounds[1][i]])
            actions_scale[:,i] = action_temp#.reshape(self.batch_size,1)
            # clip action 2018-01-14
            actions_scale[:, i] = np.clip(actions_scale[:, i], self.action_bounds[0][i], self.action_bounds[1][i])
    ...
    def store_transition(self, s, a, r, s_, d):
        ...
        self.ob_normalize2.update(np.resize(s, [1, self.state_dim]))#TODO test observation normalization
        self.replay_buffer_count += 1
        self.replay_buffer_count = min(self.replay_buffer_count,self.replay_buffer_size)
