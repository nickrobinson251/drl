...
class CriticNetwork:
    ...
    def create_training_method(self):
        ...
        if self.is_prioritized_replay == True:
            ...
            self.loss = tf.reduce_mean(self.ISWeights * tf.square(self.y_input - self.q_value_output))
            ...
            self.loss = tf.reduce_mean(tf.square(self.y_input - self.q_value_output))
            self.cost = self.loss + weight_decay
    ...
    def create_q_network(self, state_dim, action_dim, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            if self.is_layer_norm == True:
                ...
                #layer1_norm = tf.nn.relu(layer1_norm)seed = int(datetime.now().strftime('%S%f'))
