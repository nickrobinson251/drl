...
class calCOP():
    def __init__(self, force_cuttoff_freq, pos_cutoff_freq, dt, order):
        self.left_foot_force_filter = [FilterClass(),FilterClass(),FilterClass()]
        self.right_foot_force_filter = [FilterClass(),FilterClass(),FilterClass()]
        self.left_foot_COP_filter = [FilterClass(),FilterClass(),FilterClass()]
        self.right_foot_COP_filter = [FilterClass(),FilterClass(),FilterClass()]
        self.COP_filter = [FilterClass(),FilterClass(),FilterClass()]
        self.force_filter = [FilterClass(),FilterClass(),FilterClass()]

        for i in range(3):
            self.left_foot_force_filter[i].butterworth(dt, force_cuttoff_freq, order)
            self.right_foot_force_filter[i].butterworth(dt, force_cuttoff_freq, order)
            ...
            self.right_foot_COP_filter[i].butterworth(dt, pos_cutoff_freq, order)
    ...
    def __call__(self, right_contact_info, left_contact_info):
        ...
        rightFootCOP, rightF, rightFootCOPFlag = self.calFootCOP(right_contact_info)
        self.right_COP_info = [rightFootCOP, rightF, rightFootCOPFlag]

        if leftFootCOPFlag == True and rightFootCOPFlag == False:
            COP = leftFootCOP
            F = leftF
            COPFlag = True
        elif leftFootCOPFlag == False and rightFootCOPFlag == True:
            COP = rightFootCOP
            F = rightF
            COPFlag = True
        elif leftFootCOPFlag == True and rightFootCOPFlag == True:
            F = leftF + rightF
        ...
        return COP, F, COPFlag, rightFootCOP, rightF, rightFootCOPFlag, leftFootCOP, leftF, leftFootCOPFlag
    ...
    def calFootCOP(self, contact_info):
        ...
        for i in range(len(contact_info)):
            ...
            F = F + F_contact
            COP = COP + contactPosition * F_contact[2]  # integration of contact point times vertical force
        COP = COP / F[2]
    ...
    def getFilteredCOP(self):
        COP = np.array([self.COP_filter[0].y[0], self.COP_filter[1].y[0], self.COP_filter[2].y[0]])
        COPFlag = self.COP_info[1]
        F = np.array([self.force_filter[0].y[0], self.force_filter[1].y[0], self.force_filter[2].y[0]])

        leftFootCOP = np.array([self.left_foot_COP_filter[0].y[0], self.left_foot_COP_filter[1].y[0], self.left_foot_COP_filter[2].y[0]])
        leftFootCOPFlag = self.left_COP_info[1]
        leftF = np.array([self.left_foot_force_filter[0].y[0], self.left_foot_force_filter[1].y[0], self.left_foot_force_filter[2].y[0]])

        rightFootCOP = np.array([self.right_foot_COP_filter[0].y[0], self.right_foot_COP_filter[1].y[0], self.right_foot_COP_filter[2].y[0]])
        rightFootCOPFlag = self.right_COP_info[2]
        rightF = np.array([self.right_foot_force_filter[0].y[0], self.right_foot_force_filter[1].y[0], self.right_foot_force_filter[2].y[0]])
