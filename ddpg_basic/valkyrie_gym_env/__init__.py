...
class Valkyrie(gym.Env):
    ...
    def _reset(self, Kp=Kp_default, Kd=Kd_default, base_pos_nom = None, base_orn_nom = None, fixed_base = False):
        self.Kp = Kp
        self.Kd = Kd
    ...
    def _step(self, action, pelvis_push_force=[0,0,0]):
        ...
        for i in range(int(self._actionRepeat)):  # higher frequency for physics simulation
            p.applyExternalForce(self.r, -1, forceObj=pelvis_push_force, posObj=[0, 0.0035, 0],
    ...
    def _setupFilter(self):
        # filtering for COP
        self.COP_filter_method = calCOP(10,10,self._dt_filter,1) #TODO test calculation and filtering for COP
    ...
    def _setupCamera(self, cameraDistance=1.5, cameraYaw=0, cameraPitch=0, cameraTargetPosition=[0, 0, 0.7]):
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 1)
    ...
    def _setupSimulation(self, base_pos_nom=None, base_orn_nom=None, fixed_base=False):
        if base_pos_nom is None:
            base_pos_nom = self.base_pos_nom
        if base_orn_nom is None:
            base_orn_nom = self.base_orn_nom
        ...
        self.plane = p.loadURDF(plane_urdf, basePosition=[0, 0, 0], useFixedBase=True)
        # valkyrie_urdf = self.dir_path + "/valkyrie_bullet_mass_sims_huge_foot.urdf"
        # valkyrie_urdf = self.dir_path + "/valkyrie_bullet_mass_sims.urdf"
        #valkyrie_urdf = self.dir_path + "/valkyrie_bullet_mass_sims_modified_foot_collision_box_soft_contact.urdf"#"/valkyrie_bullet_mass_sims_modified_foot_collision_box_soft_contact.urdf"
        valkyrie_urdf = self.dir_path + "/valkyrie_bullet_mass_sims_modified_foot_collision_box_modified_self_collision.urdf"
...
#        p.enableJointForceTorqueSensor(self.r, self.jointIdx['rightAnklePitch'],True)
    ...
    def calCOMPos(self):
        ...
        right_foot_info = p.getLinkState(self.r, self.jointIdx['rightAnkleRoll'], computeLinkVelocity=0)
        left_foot_info = p.getLinkState(self.r, self.jointIdx['leftAnkleRoll'], computeLinkVelocity=0)
...
#        left_foot_bottom_center = left_foot_info[4] + left_T1.T
    ...
    def calCOP(self):
        ...
        result = self.COP_filter_method(right_contact_info, left_contact_info) # right then left
