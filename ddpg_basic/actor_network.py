...
class ActorNetwork:
    ...
    def __init__(self, sess, state_dim, action_dim, config):
        self.param_noise = AdaptiveParamNoise(config.conf['param-noise-settings'][0],config.conf['param-noise-settings'][1],config.conf['param-noise-settings'][2])
    ...
    def get_perturbed_actor_updates(self, actor, perturbed_actor, param_noise_stddev):
        ...
        for var, perturbed_var in zip(actor['vars'], perturbed_actor['vars']):
            if var in actor['perturbable_vars']:
                updates.append(
                    tf.assign(perturbed_var, var + tf.random_normal(tf.shape(var), mean=0., stddev=param_noise_stddev, seed=seed)))#set seed to avoid repeating random
            else:
    ...
    def variable(self, shape, f):
        #seed = int(datetime.now().strftime('%S%f'))
        #return tf.Variable(tf.random_uniform(shape, -1 / math.sqrt(f), 1 / math.sqrt(f), seed=seed))
    ...
    def transfer_variable(self, var_dict):
        ...
        self.sess.run(self.target_init_updates) # copy network weights to target network
