...
flags.DEFINE_integer(
    'replay_batch_size',
    None,
    'replay batch size; defaults to batch_size')
...
flags.DEFINE_bool(
    'use_online_batch',
    True,
    'train on batches as they are sampled')
...
flags.DEFINE_string(
    'load_path',
    '',
    'path of saved model to load (if none in save_dir)')
...
class Trainer(object):
    ...
    def __init__(self):
        ...
        self.env = gym_wrapper.GymWrapper(
            self.env_str,
    ...
    def get_replay_buffer(self):
        if self.replay_buffer_freq <= 0:
            ...
            assert self.objective in [
                'pcl', 'upcl'], 'Can\'t use replay buffer with %s' % (
                self.objective)
