...
class Fcnn(Net):
    ...
    #             weights.append(tf.Variable(tf.truncated_normal([dim_1, dim_2], stddev=init_w), name='theta_%i' % i))
    #             # zero initialization for bias
    #             init_b = self.initialize_bias[i]
    #             if init_b is None:
    #                 bias.append(tf.Variable(np.zeros([1, dim_2]).astype(np.float32), name='bias_%i' % i))
    #             else:
    #                 bias.append(tf.Variable(tf.truncated_normal([dim_2], stddev=init_b), name='bias_%i' % i))
    #             net = self.activation_fns_call[i](tf.matmul(net, weights[i]) + bias[-1])
    #
    #         else:
    #             # print(dim_1,dim_2)
    #             init_v = self.initialize_weight[i] if self.initialize_weight is not None else .1
